/**
 * Product manifest for the Web3 Summit attendee app (bulletin-deploy).
 *
 * `domain` comes from VITE_DOTNS_ID at build time so one manifest works across
 * every environment; bulletin-deploy rejects a deploy unless it matches the
 * deploy domain.
 *
 * Not shipped as-is: `scripts/emit-manifest.mjs` evaluates this after the build
 * (with VITE_DOTNS_ID set) and writes a self-contained `out/bulletin-deploy.config.mjs`
