 ...
 * (which sidesteps PAPI's PJS adapter that throws on unknown signed-extensions
 * like previewnet's AsPgas), and delegates the actual byte signing to the
 * browser extension via `signRaw({ type: 'payload' })`.
 *
 * Requires the wallet to support `signRaw` with `type: 'payload'`
 * (Talisman, polkadot-js, SubWallet all do).
 */
...
export interface CreateStandaloneTxSignerOpts {
  ...
  /**
   * Keypair scheme as reported by the extension. Determines the MultiSignature
   * prefix byte. Defaults to sr25519.
   */
  keypairType?: string
}
