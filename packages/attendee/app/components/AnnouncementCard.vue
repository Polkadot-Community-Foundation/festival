...
<template>
  <div
    ...
    :class="props.highlight ? 'bg-surface-3' : 'bg-surface-2'"
  >
    <img
    ...
    <div class="flex-1 min-w-0">
      ...
      <div class="flex items-center gap-1.5 text-sm text-text-muted">
        ...
        <span v-else-if="loading" class="h-3 bg-surface-4 rounded animate-pulse w-28" />
      </div>
      <p v-if="body" class="text-base text-white whitespace-pre-wrap break-words leading-snug">
        {{ body.content }}
      </p>
      <div v-else-if="loading" class="space-y-2 mt-2">
        <div class="h-3 bg-surface-4 rounded animate-pulse w-3/4" />
        <div class="h-3 bg-surface-4 rounded animate-pulse w-1/2" />
      </div>
      <p v-else class="text-xs text-text-muted">Couldn't load announcement.</p>
      <p v-if="body?.senderName" class="text-xs text-text-muted mt-2">
        {{ body.senderName }}
      </p>
    </div>
  </div>
